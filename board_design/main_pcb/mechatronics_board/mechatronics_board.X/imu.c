...
void imu_setup()
{
  ...
  if (who != 0b01101001)
  { // will stay in while loop if I2C bus isn't working
    while (1)
    {
  ...
  writePin(
      IMU_ADDR, IMU_CTRL1_XL,
      0b10000010); // 1000 => 1.66kHz, 00 => +/- 2g sensitivity, 10 => 100 Hz)
...
void bar_x(signed short xaccel)
{
  ...
  for (i = 0; i < 100; i++)
  { // go across 100 pixels
    for (j = 0; j < 10; j++)
    { // width of bar
      if (x < 0)
      { // screen is tilting left, -i
        if (i > fabs(x))
        { // if magnitude of x is bigger than index
...
void bar_y(signed short yaccel)
{
  ...
  { // go across 100 pixels
    ...
    { // width of bar
      if (y > 0)
      { // screen is tilting away from user, -i
        if (i > fabs(y))
        { // if magnitude of y is bigger than index
