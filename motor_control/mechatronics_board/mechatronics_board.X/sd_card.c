...
int initSD() {
    ...
    SPI1BRG = 95;           // set SPI baud rate to 250kHz until SD card properly initialized
    ...
    SD_CS = 1;
    if (r != 1) { // return command error if IDLE not returned
        return COMD_ERR;
    }
    ...
    for (i = 0; i < 10000; i++) {
        ...
        if (!r) {                                       // break loop once IDLE ends (command response = 0)
            SD_CS = 1;
            break;
        }
    }
        ...
        if ((read & 0x1F) == DAT_ACC) {
            *token = DAT_ACC; // set token to data accepted
            ...
            while (spi_io(0xFF) == 0x00) {
                if (TMR4 > 46874) {         // if timeout reached
