...
// build an array of high/low times from the color input array, then output the
// high/low bits
void ws2812b_setColor(wsColor *c, int numLEDs) {
  ...
  int numBits =
      2 * 3 * 8 *
      numLEDs; // the number of high/low bits to store, 2 per color bit
  volatile unsigned int
      delay_times[2 * 3 * 8 * numLEDs]; // I only gave you 5 WS2812B, adjust
                                        // this if you get more somewhere

  // start at time at 0
  delay_times[0] = 0;

  int nB = 1; // which high/low bit you are storing, 2 per color bit, 24 color
              // bits per WS2812B
