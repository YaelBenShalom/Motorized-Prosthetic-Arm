...
def main(args):
    ...
    if use_my_data:
        ...
        m_body = 53
        # Average upper arm weights relative to body weight, from “Biomechanics
        m_u = 0.028 * m_body
        # and Motor Control of Human Movement” by David Winter (2009), 4th edition
        # Average lower prosthetics weights, calculated using lab measurements
        m_l = 0.7395
        ...
        H_body = 1.62
        # Average upper arm length relative to body height
        L_u = 0.186 * H_body
        ...
        # from “Biomechanics and Motor Control of Human Movement” by David
        # Winter (2009), 4th edition
        # Average lower prosthetics length, calculated using lab measurements
        L_l = 0.42

        # Arm center of mass length calculations (length unit is m)
        # Average upper arm length from shoulder to center of mass relative
        L_u_c = 0.436 * L_u
        # to upper arm length, from “Biomechanics and Motor Control of Human
        # Movement” by David Winter (2009), 4th edition
        # Average lower prosthetics length from elbow to center of mass,
        L_l_c = 0.2388
        ...
        # Arm length calculations (length unit is m)
        # Average height for American adult male, from “Height and body-mass
        H_body = 1.769
                    ...
                    if "00B429F8" in file:
                        if file.endswith(".csv"):
                            ...
                            data_path = os.path.join(
                                data_csv_dir, folder, file)
                            data_rows = open(data_path).read(
                            ).strip().split("\n")[6000:7500]
                            ...
                            for row in data_rows:
                                ...
                                file_shoulder_ang_list.append(
                                    float(splitted_row[31]) * 2*pi/360)
                                file_shoulder_acc_data_list.append(
                                    float(splitted_row[14]))
                                frame += 1

                            # Extract elbow and shoulder velocities [rad/sec] from angles
                            for i in range(len(file_time_list) - 1):
                                shoulder_vel = calculate_vel(
                                    file_shoulder_ang_list, file_time_list, i)
                            ...
                            for i in range(len(file_time_list) - 2):
                                shoulder_acc = calculate_acc(
                                    file_shoulder_vel_list, file_time_list, i)
                            ...
                            shoulder_ang_list.append(
                                adjusted_file_shoulder_ang_list)
                            shoulder_vel_list.append(
                                adjusted_file_shoulder_vel_list)
                            shoulder_acc_list.append(file_shoulder_acc_list)
                            shoulder_acc_data_list.append(
                                adjusted_file_shoulder_acc_data_list)
                            break
                    ...
                    if "00B429E2" in file:
                        if file.endswith(".csv"):
                            ...
                            for i in range(len(data_rows)):
                                ...
                                file_time_list.append(frame / frame_frequency)
                                file_tot_ang_list.append(
                                    float(splitted_row[31]) * 2*pi/360)
                                file_elbow_ang_list.append(
                                    (float(splitted_row[31]) - file_shoulder_ang_list[i]) * 2*pi/360)
                                file_elbow_acc_data_list.append(
                                    float(splitted_row[14]))
                                frame += 1

                            # Extract elbow and shoulder velocities [rad/sec] from angles
                            for i in range(len(file_time_list) - 1):
                                elbow_vel = calculate_vel(
                                    file_elbow_ang_list, file_time_list, i)
                                file_elbow_vel_list.append(elbow_vel)

                            # Extract elbow and shoulder Accelerations [rad/sec^2] from velocities
                            for i in range(len(file_time_list) - 2):
                                elbow_acc = calculate_acc(
                                    file_elbow_vel_list, file_time_list, i)
                            ...
                            elbow_acc_data_list.append(
                                file_elbow_acc_data_list)
                            break
                    ...
                    if "00B43D0C" in file:
                        if file.endswith(".csv"):
                            ...
                            for i in range(len(data_rows)):
                                ...
                                file_back_ang_list.append(
                                    float(splitted_row[31]) * 2*pi/360)
                                file_back_pos_list.append(
                                    float(splitted_row[21]))
                                file_back_vel_list.append(
                                    float(splitted_row[24]))
                                frame += 1
        ...
        for file in os.listdir(data_csv_dir):
            ...
            if file.endswith(".csv"):
                ...
                for row in data_rows:
                    ...
                    file_time_list.append(frame / frame_frequency)
                    file_R_shoulder_ang_list.append(
                        float(splitted_row[11]) * 2*pi/360)
                    file_R_elbow_ang_list.append(
                        float(splitted_row[9]) * 2*pi/360)
                    file_L_shoulder_ang_list.append(
                        float(splitted_row[23]) * 2*pi/360)
                    file_L_elbow_ang_list.append(
                        float(splitted_row[21]) * 2*pi/360)
                    frame += 1

                # Extract elbow and shoulder velocities [rad/sec] from angles
                for i in range(len(file_time_list) - 1):
                    R_elbow_vel = calculate_vel(
                        file_R_elbow_Ang_list, file_time_list, i)
                    R_shoulder_vel = calculate_vel(
                        file_R_shoulder_ang_list, file_time_list, i)
                    L_elbow_vel = calculate_vel(
                        file_L_elbow_ang_list, file_time_list, i)
                    L_shoulder_vel = calculate_vel(
                        file_L_shoulder_ang_list, file_time_list, i)
                ...
                # Extract elbow and shoulder Accelerations [rad/sec^2] from velocities
                for i in range(len(file_time_list) - 2):
                    R_elbow_acc = calculate_acc(
                        file_R_elbow_vel_list, file_time_list, i)
                    R_shoulder_acc = calculate_acc(
                        file_R_shoulder_vel_list, file_time_list, i)
                    L_elbow_acc = calculate_acc(
                        file_L_elbow_vel_list, file_time_list, i)
                    L_shoulder_acc = calculate_acc(
                        file_L_shoulder_vel_list, file_time_list, i)
    ...
    if not use_double_pendulum:
        ...
        solution_subs = solution_subs.subs(
            [((theta.diff(t)).diff(t), theta_ddot_dummy)])
        ...
        for i in range(len(time_list)):
            ...
            for j in range(len(t_list)):
                tau_list.append(
                    func(theta_list[j], dtheta_list[j], ddtheta_list[j], m_l, L_l, L_l_c))
        ...
        print(
            f"maximum elbow angle is {format(max_elbow_ang, '.3f')} [rad], in velocity {walking_vel_list[max_elbow_ang_index]} (trial {max_elbow_ang_index})")
        print(
            f"maximum elbow angular velocity is {format(max_elbow_vel, '.3f')} [rad/s] ({format(max_elbow_vel*60/(2*pi), '.3f')} [rpm]), in velocity {walking_vel_list[max_elbow_vel_index]} (trial {max_elbow_vel_index})")
        print(
            f"maximum elbow torque is {format(max_elbow_tau, '.3f')} [Nm], in velocity {walking_vel_list[max_elbow_tau_index]} (trial {max_elbow_tau_index})")
        print(
            f"maximum elbow power is {format(max_elbow_power, '.3f')} [W], in velocity {walking_vel_list[max_elbow_power_index]} (trial {max_elbow_power_index})")

        # The torque equations for the maximum power:
        solution_subs = solution_subs.subs(
            [(m, m_l), (R, L_l), (R_c, L_l_c), (g, 9.81)])
    ...
    if use_double_pendulum:
        # Computing the Lagrangian of the system
        m1, m2, g, R1, R1_c, R2, R2_c = symbols(
            r'm1, m2, g, R1, R1_c, R2, R2_c')
        ...
        # Calculating the kinetic and potential energy of the system
        KE = 1/2 * m1 * ((x1.diff(t))**2 + (y1.diff(t))**2) + \
        ...
        for i in range(len(time_list)):
            ...
            freq_list.append(
                1 / ((vel_freq_list[1] - vel_freq_list[0]) / frame_frequency))
        ...
        if use_my_data:
            ...
            solution_0_subs = solution_0_subs.subs([(m1, m_u_dict[participants_list[max_elbow_tau_index]]),
                                                    (m2, m_l), (
                                                        R1, L_u_dict[participants_list[max_elbow_tau_index]]),
                                                    (R2, L_l), (
                                                        R1_c, L_u_c_dict[participants_list[max_elbow_tau_index]]),
                                                    (R2_c, L_l_c), (g, 9.81)])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--double_pendulum", default=False,
                        help="use double pendulum")
