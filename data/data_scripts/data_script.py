...
def main():
    """ Calculates angles, velocities, and acceleration form data
    """
    ...
    for file in os.listdir(data_csv_dir):
        if file.endswith(".csv"):
            ...
            headers_list.append([file, splitted_headers[9], splitted_headers[11],
                                splitted_headers[21], splitted_headers[23]])
            ...
            for i in range(len(file_time_list) - 1):
                ...
                R_Shl_Flex_Vel = calculate_Vel(
                    file_R_Shl_Flex_Ang_list, file_time_list, i)
                L_Elbow_Vel = calculate_Vel(
                    file_L_Elbow_Ang_list, file_time_list, i)
                L_Shl_Flex_Vel = calculate_Vel(
                    file_L_Shl_Flex_Ang_list, file_time_list, i)
            ...
            for i in range(len(file_time_list) - 2):
                ...
                R_Shl_Flex_Acc = calculate_Acc(
                    file_R_Shl_Flex_Vel_list, file_time_list, i)
                L_Elbow_Acc = calculate_Acc(
                    file_L_Elbow_Vel_list, file_time_list, i)
                L_Shl_Flex_Acc = calculate_Acc(
                    file_L_Shl_Flex_Vel_list, file_time_list, i)
